  ...
  switch (contentType) {
    case ContentType.Script:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are an expert scriptwriter for engaging social media videos.`,
      );
      return {
        prompt: `${baseDetails}Request: Generate a compelling video script. The script should be tailored for ${platform}, including an introduction, main content points, and a call to action (if implicitly requested or appropriate). Ensure the tone and length are appropriate. If a target audience is specified, deeply consider their preferences.`,
        systemInstruction,
      };
    case ContentType.Idea:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a creative content strategist specializing in viral social media trends.`,
      );
      return {
        prompt: `${baseDetails}Request: Brainstorm ${batchVariations && batchVariations > 1 ? batchVariations : 3} unique and engaging content ideas for ${platform}. For each idea, provide a catchy hook, a brief concept outline, and any relevant suggestions for format or style. Present each idea clearly separated.`,
        systemInstruction,
      };
    ...
    case ContentType.ThumbnailConcept:
      if (isABTesting && abTestType) {
        ...
        switch (abTestType) {
          case ABTestableContentType.Title:
            systemInstruction = getSystemInstructionFromDefinition(
              aiPersonaDef,
              `You are a master copywriter specializing in A/B testing social media titles.`,
            );
          ...
          case ABTestableContentType.VideoHook:
            systemInstruction = getSystemInstructionFromDefinition(
              aiPersonaDef,
              `You are an expert in A/B testing attention-grabbing video hooks.`,
            );
          ...
          case ABTestableContentType.ThumbnailConcept:
            systemInstruction = getSystemInstructionFromDefinition(
              aiPersonaDef,
              `You are a visual strategist specializing in A/B testing thumbnail concepts.`,
            );
      ...
      if (contentType === ContentType.Title)
        return {
          prompt: `${baseDetails}Request: Generate ${batchVariations && batchVariations > 1 ? batchVariations : 5} catchy and effective titles/headlines suitable for a ${platform} post. Optimize for attention and clicks/views. Present each title clearly separated.`,
          systemInstruction,
        };
      if (contentType === ContentType.VideoHook)
        return {
          prompt: `${baseDetails}Request: Generate ${batchVariations && batchVariations > 1 ? batchVariations : 3} short, compelling video hooks (first 3-10 seconds) for a video about "${userInput}" on ${platform}. Hooks should immediately capture viewer interest. Present each hook clearly separated.`,
          systemInstruction,
        };
      if (contentType === ContentType.ThumbnailConcept)
        return {
          prompt: `${baseDetails}Request: For a video on ${platform} about "${userInput}", provide ONE concept for a compelling thumbnail. Include:
...
Ensure the concept is visually appealing and relevant.`,
          systemInstruction,
        };
      break;
    ...
    case ContentType.ImagePrompt:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are an AI assistant generating highly detailed and creative prompts for image generation models.`,
      );
      return {
        prompt: `${baseDetails}Request: Create ${batchVariations && batchVariations > 1 ? batchVariations : 1} detailed text prompt(s) an AI image generator can use based on the core concept: "${userInput}". Include art style, mood, key visual elements, color palette, and composition. If negative prompts are implied by user input or good practice, include them. Prompt should be descriptive for specific imaginative outcomes. If multiple prompts, ensure distinct interpretations.`,
        systemInstruction,
      };
    ...
    case ContentType.TrendAnalysis:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a Trend Analyst AI. Your task is to identify and summarize current trends, news, and discussions for a given niche using Google Search. You will provide a textual summary and the API will return grounding sources.`,
      );
...
`,
    ...
    case ContentType.ContentBrief:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a strategic content planner AI.",
      );
      outputConfig.responseMimeType = "application/json";
      return {
    ...
    case ContentType.PollsQuizzes:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an expert in creating engaging social media interactions.",
      );
      outputConfig.responseMimeType = "application/json";
      return {
...
For quizzes, items are objects: {"question": "string", "options": ["string", ...], "correctAnswerIndex": number, "explanation": "optional string"}.`,
    ...
    case ContentType.ContentGapFinder:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a market research AI specializing in content strategy and SEO.",
      );
      outputConfig.tools = [{ googleSearch: {} }];
      return {
    ...
    case ContentType.ChannelAnalysis:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a world-class YouTube strategy consultant and analyst. Your analysis should be in-depth, actionable, and data-informed using Google Search. If multiple channels are provided, conduct a comparative analysis where appropriate.",
      );
      outputConfig.tools = [{ googleSearch: {} }];
      return {
...
`,
    ...
    case ContentType.MicroScript:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an expert in crafting concise and impactful short-form video scripts.",
      );
      return {
...
Label each part clearly.`,
    ...
    case ContentType.VoiceToScript:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an AI assistant specialized in transforming raw voice transcriptions into polished, engaging scripts.",
      );
      const inputTypeMessage = isVoiceInput
        ? "The following is a raw transcription from spoken audio."
        : "The following is user-provided text, potentially from a transcription or rough notes.";
      return {
    ...
    case ContentType.ContentStrategyPlan:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an expert Content Strategist AI. Your goal is to provide comprehensive, actionable content strategy plans.",
      );
      outputConfig.responseMimeType = "application/json";
      ...
      if (strategyInputs) {
        ...
        if (strategyInputs.targetAudience)
          strategyPrompt += `Target Audience Description: ${strategyInputs.targetAudience}\n`;
        if (strategyInputs.goals && strategyInputs.goals.length > 0)
          strategyPrompt += `Main Goals: ${strategyInputs.goals.join(", ")}\n`;
        if (strategyInputs.platforms && strategyInputs.platforms.length > 0)
          strategyPrompt += `Target Platforms: ${strategyInputs.platforms.join(", ")}\n`;
      } else {
...
`;
    ...
    case ContentType.EngagementFeedback:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an AI expert in social media engagement and content analysis. Your feedback should be constructive, specific, and actionable.",
      );
      return {
    ...
    case ContentType.RefinedText:
      if (!originalText || !refinementType)
        return { prompt: "Invalid refinement request." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a meticulous text editing assistant.`,
      );
      return {
        prompt: `Original Text: "${originalText}"\nRefinement Request: ${refinementType}\nPlatform Context: ${platform}\nTarget Audience (if any): ${targetAudience || "General"}\nTask: Rewrite the original text according to the refinement request. Maintain the core message but adjust as specified. Ensure output is appropriate for platform and audience.`,
        systemInstruction,
      };
    case ContentType.Hashtags:
      if (!originalText)
        return { prompt: "No text provided for hashtag generation." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a social media hashtag expert.`,
      );
      return {
        prompt: `Content: "${originalText}"\nPlatform: ${platform}\nRequest: Generate a list of 10-15 relevant and effective hashtags for this content on ${platform}. Include a mix of broad, niche, and potentially trending hashtags. Present them as a comma-separated or space-separated list.`,
        systemInstruction,
      };
    case ContentType.Snippets:
      if (!originalText)
        return { prompt: "No text provided for snippet extraction." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a skilled content summarizer and teaser writer.`,
      );
      return {
        prompt: `Full Content: "${originalText}"\nPlatform: ${platform}\nRequest: Extract 2-3 short, catchy snippets from the full content suitable for teaser posts or social media updates on ${platform}. Each snippet should be engaging and standalone.`,
        systemInstruction,
      };
    case ContentType.RepurposedContent:
      if (
        ...
        return { prompt: "Invalid repurposing request." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are a content adaptation specialist.`,
      );
      return {
        prompt: `Original Content (from ${platform}, about "${userInput}"): "${originalText}"\nRequest: Repurpose this content for ${repurposeTargetPlatform} as a ${repurposeTargetContentType}. Adapt style, format, length, and tone to be optimal for the target platform and content type. Target Audience (if any): ${targetAudience || "General"}\nNew Output Should Be: A complete ${repurposeTargetContentType} for ${repurposeTargetPlatform}.`,
        systemInstruction,
      };

    case ContentType.VisualStoryboard:
      if (!originalText)
        return { prompt: "No script provided for storyboard generation." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an AI assistant that helps create visual storyboards from scripts.",
      );
      return {
        prompt: `Video Script: "${originalText}"\nRequest: Based on the provided script, suggest 5-7 key visual storyboard points. For each point, describe the scene, suggested camera shot/angle, and any important on-screen elements or graphics. Focus on visually representing the script's narrative.`,
        systemInstruction,
      };
    case ContentType.MultiPlatformSnippets:
      if (
        !originalText ||
        !multiPlatformTargets ||
        multiPlatformTargets.length === 0
      )
        return { prompt: "Invalid multi-platform snippet request." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an expert in cross-platform content adaptation.",
      );
      return {
        prompt: `Original Content (about "${userInput}"): "${originalText}"\nTarget Platforms: ${multiPlatformTargets.join(", ")}\nRequest: For each target platform, generate one short, engaging snippet (e.g., tweet, Instagram caption, LinkedIn update) adapted from the original content. Ensure each snippet is optimized for its respective platform's style and length constraints. Clearly label snippets by platform.`,
        systemInstruction,
      };
    case ContentType.ExplainOutput:
      if (!originalText)
        return { prompt: "No output provided for explanation." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a helpful AI assistant that can explain its own reasoning.",
      );
      return {
        prompt: `AI-Generated Text: "${originalText}"\nUser's Original Prompt that led to this text: "${userInput}"\nRequest: Briefly explain the creative choices, patterns, or reasoning that led to the generation of the AI-Generated Text based on the Original Prompt. What aspects of the prompt influenced the output most? Keep it concise (2-3 sentences).`,
        systemInstruction,
      };
    case ContentType.FollowUpIdeas:
      if (!originalText)
        return {
          prompt: "No existing content provided to generate follow-up ideas.",
        };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a creative strategist skilled in developing content series.",
      );
      return {
        prompt: `Existing Content (on ${platform}, about "${userInput}"): "${originalText}"\nRequest: Brainstorm 3 distinct ideas for follow-up content or a content series based on the existing content. For each idea, suggest a title/hook and a brief concept. Consider what the audience might want to see next.`,
        systemInstruction,
      };
    case ContentType.SeoKeywords:
      if (!originalText && seoMode === SeoKeywordMode.Suggest)
        return { prompt: "No content provided to suggest SEO keywords from." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an SEO specialist AI.",
      );
      if (seoMode === SeoKeywordMode.Suggest) {
        return {
          prompt: `Content Text: "${originalText}"\nRequest: Analyze this text and suggest 5-7 relevant SEO keywords or keyphrases. Focus on terms that users might search for related to this content.`,
          systemInstruction,
        };
      }
...
Output MUST be a valid JSON array of objects, each with "id" (string, unique), "suggestedPrompt" (string), and "reasoning" (string, optional). Example: [{"id": "opt1", "suggestedPrompt": "Improved prompt...", "reasoning": "This version adds..."}]`,
    ...
    case ContentType.YouTubeDescription:
      if (!originalText)
        return {
          prompt:
            "No YouTube title/script provided for description optimization.",
        };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are an expert YouTube SEO and content packager.",
      );
      return {
    ...
    case ContentType.TranslateAdapt:
      if (!originalText || !targetLanguage)
        return { prompt: "Missing text or target language for translation." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are an expert multilingual translator and cultural adaptation specialist.`,
      );
      return {
    ...
    case ContentType.CheckReadability:
      if (!originalText)
        return { prompt: "No text provided for readability check." };
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        "You are a language and readability analysis expert.",
      );
    ...
    case ContentType.YoutubeChannelStats:
      systemInstruction = getSystemInstructionFromDefinition(
        aiPersonaDef,
        `You are an expert in finding and presenting key statistics for YouTube channels using available information.`,
      );
      outputConfig.tools = [{ googleSearch: {} }];
      // DO NOT use responseMimeType: "application/json" with googleSearch tool.
      return {
  ...
  return {
    prompt: `Please provide content for ${contentType} on ${platform} about ${userInput}.`,
    systemInstruction: getSystemInstructionFromDefinition(aiPersonaDef),
  };
};
...
// Mock content generator for when API key is invalid
...
A professional, high-quality image featuring ${userInput}. The scene should be vibrant and engaging, with perfect lighting and composition. Include elements that represent innovation, success, and creativity. Style: modern, clean, eye-catching. Perfect for ${platform} content.`,
  ...
  return {
    ...
    content:
      ...
      `Mock AI-generated content for "${userInput}" on ${platform}. This is a sample response that would be created by the Gemini AI when properly configured.`,
  };
};
...
export const generateTextContent = async (
  ...
  try {
    ...
    const response: GenerateContentResponse =
      await currentAI.models.generateContent(requestConfig);
    ...
    console.error(
      `Error generating text content for ${options.contentType}:`,
      error,
    );
    if (error instanceof Error) {
      // Specific check for the TrendAnalysis bug if it still somehow occurs before UI parsing
      if (
        options.contentType === ContentType.TrendAnalysis &&
        error.message.includes(
          "Tool use with a response mime type: 'application/json' is unsupported",
        )
      ) {
        throw new Error(
          `Trend Analysis API configuration error: 'application/json' mime type is not supported with Google Search. The prompt has been updated to request plain text. Please check parsing logic if this persists. Original error: ${error.message}`,
        );
      }
      throw new Error(
        `Failed to generate text for ${options.contentType}: ${error.message}`,
      );
    }
    throw new Error(
      `An unknown error occurred while generating text for ${options.contentType}.`,
    );
  }
};
...
export const generateImage = async (
  ...
  if (aspectRatioGuidance && aspectRatioGuidance !== AspectRatioGuidance.None) {
    ...
    switch (aspectRatioGuidance) {
      case AspectRatioGuidance.SixteenNine:
        aspectRatioText =
          "Ensure the image is in 16:9 aspect ratio (wide landscape).";
        break;
      case AspectRatioGuidance.NineSixteen:
        aspectRatioText =
          "Ensure the image is in 9:16 aspect ratio (tall portrait).";
        break;
      case AspectRatioGuidance.OneOne:
        aspectRatioText = "Ensure the image is in 1:1 aspect ratio (square).";
        break;
      case AspectRatioGuidance.FourFive:
        aspectRatioText = "Ensure the image is in 4:5 aspect ratio (portrait).";
        break;
      case AspectRatioGuidance.ThreeTwo:
        aspectRatioText =
          "Ensure the image is in 3:2 aspect ratio (landscape photo).";
        break;
      case AspectRatioGuidance.TwoThree:
        aspectRatioText =
          "Ensure the image is in 2:3 aspect ratio (portrait photo).";
        break;
    }
...
export const performWebSearch = async (
  ...
  const currentAI = getAIInstance();
  let searchRequestType = isRequestingMore
    ? "Find *additional and different* websites, articles, or resources"
    : "Find websites, articles, or resources";

  let searchPrompt = `${searchRequestType} related to "${query}".`;

  if (fileExtension && fileExtension.trim() !== "") {
    const ext = fileExtension.trim().startsWith(".")
      ? fileExtension.trim()
      : `.${fileExtension.trim()}`;
  ...
  try {
    ...
    if (response.candidates?.[0]?.groundingMetadata?.groundingChunks) {
      sources = response.candidates[0].groundingMetadata.groundingChunks
        .filter(
          (chunk) => chunk.web && chunk.web.uri && chunk.web.uri.trim() !== "",
        )
        .map((chunk) => ({
          uri: chunk.web!.uri!,
      ...
      if (matches) {
        matches.forEach((url) => {
          ...
          const titleRegex = new RegExp(
            `(?:[""']|(?:title is )|(?:called ))([^"""']{5,80})(?:[""""]| for ${url.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`,
            "i",
          );
          ...
          if (
            !sources.some((existing) => existing.uri === url) &&
            !textUrls.some((existing) => existing.uri === url)
          ) {
