...
export const App = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (!process.env.GEMINI_API_KEY) {
      ...
      setError(
        "Configuration error: GEMINI_API_KEY is missing. Please ensure it's set in your environment variables.",
      );
    }
  }, []);
        ...
        if (effectiveContentType === ContentType.Image) {
          ...
          try {
            ...
            if (apiError.message?.includes("INVALID_API_KEY")) {
              // Use mock content when API key is invalid
  ...
  const handleGenerateContent = useCallback(() => {
    if (apiKeyMissing) {
      setError("Cannot generate content: GEMINI_API_KEY is missing.");
      return;
    }
  ...
  const handlePerformWebSearch = useCallback(
    async (isLoadMore = false) => {
      if (apiKeyMissing) {
        setSearchError("Cannot perform search: GEMINI_API_KEY is missing.");
        return;
      }
      if (!searchQuery.trim()) {
    ...
    if (canvasItem.type === "historyItem" && canvasItem.historyItemId) {
      ...
      return (
        ...
        >
          ...
          <div className="overflow-y-auto flex-grow min-h-0 mb-1.5 p-2 scrollbar-thin scrollbar-thumb-slate-500 scrollbar-track-slate-600/50">
            {displayContent}
          </div>{" "}
      ...
      return (
        <div
          ...
          style={{
            ...
            borderColor: canvasItem.borderColor || DEFAULT_SHAPE_BORDER_COLOR,
            borderWidth: canvasItem.borderWidth || "1px",
  ...
  const SEARCH_FILE_TYPES = useMemo(
    () => [
      ...
      ), // Filter out generic/non-specific
    ...
    if (channelNameMatch && channelNameMatch[1]) {
      ...
      if (!userInput.startsWith("http") && !userInput.startsWith("www.")) {
        ...
        if (urlMatch && urlMatch[1]) {
          ...
          const potentialChannelNameMatch = content.match(
            /^(?!https?:\/\/|www\.)([a-zA-Z0-9\s._-]+?)(?:\s+has|\s+with|\s+-\s+)/i,
          );
    ...
    <div className="min-h-screen text-slate-100 flex flex-col">
      ...
      <main className="container mx-auto p-4 flex-grow flex flex-col md:flex-row gap-6">
        {activeTab === "generator" && (
          <div className="flex-grow md:w-3/5 lg:w-2/3 bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button
                    onClick={() => handleTextAction(ContentType.OptimizePrompt)}
                  ...
                  <div className="flex items-end space-x-2">
                    <div className="flex-grow">
                      ...
                      <select
                        ...
                        onChange={(e) => setSelectedAiPersonaId(e.target.value)}
            ...
            <div
              ...
              className="flex-grow bg-slate-900/60 p-5 rounded-xl shadow-inner overflow-y-auto min-h-[200px] border border-slate-700/50 relative scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-800/50"
            >
                      ...
                      >
                        ...
                        {result.variation.type === "text" && (
                          <p className="text-sm text-slate-200 whitespace-pre-wrap my-1.5 bg-slate-600/50 p-2.5 rounded">
                            {(result.variation as GeneratedTextOutput).content}
                          </p>
                        )}
            ...
            <div className="p-2.5 border-b border-slate-700/80 flex items-center justify-between space-x-1 sm:space-x-2 flex-wrap bg-slate-800/50 shadow-sm">
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <div className="relative" ref={shapeDropdownRef}>
                  ...
                  {showShapeDropdown && (
                    <div
                      className="absolute top-full left-0 mt-1.5 w-44 bg-slate-700 border border-slate-600 rounded-md shadow-xl z-20 py-1.5"
                      role="menu"
                    >
                      {" "}
                      {CANVAS_SHAPE_VARIANTS.map((shape) => (
                        <button
                          key={shape.value}
                          onClick={() =>
                          ...
                          className="w-full text-left px-3 py-2 text-xs text-slate-200 hover:bg-sky-600 flex items-center transition-colors"
                          role="menuitem"
                        >
                          {" "}
                          {getShapeIcon(shape.value)} {shape.label}{" "}
                        </button>
                      ))}{" "}
                    </div>
                  )}
              ...
              <div className="flex items-center space-x-1 sm:space-x-2">
                ...
                <ToolbarButton
                  ...
                  className="bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white px-3 py-2 shadow-md hover:shadow-lg"
                >
                  AI Gen
                </ToolbarButton>
              </div>
        ...
        {activeTab === "channelAnalysis" && (
          <div className="flex-grow md:w-full bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-6">
            <h2 className="text-2xl font-semibold text-sky-400 mb-1 flex items-center">
              <UsersIcon className="w-7 h-7 mr-3 text-sky-400" />
              YouTube Channel Analysis
            </h2>
            <p className="text-sm text-slate-300">
              Enter YouTube channel names or URLs (comma-separated) to analyze
              for content themes, popular videos, and potential content gaps.
              This uses Google Search for analysis.
            </p>
            <div className="space-y-3">
              ...
              >
                YouTube Channel Name(s) or URL(s)
              </label>
              <textarea
                id="channelInput"
                ...
                placeholder={
                  DEFAULT_USER_INPUT_PLACEHOLDERS[ContentType.ChannelAnalysis]
                }
                rows={2}
              ...
              <button
                onClick={handleGenerateContent}
                disabled={isAnalyzingChannel || !channelAnalysisInput.trim()}
                className="px-5 py-2.5 bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out disabled:opacity-60 flex items-center space-x-2"
              >
              ...
              <div className="mb-6 p-4 bg-indigo-900/50 rounded-lg border border-indigo-700 shadow-md">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button
                    ...
                    className="text-xs text-slate-400 hover:text-slate-200 p-1 rounded-md hover:bg-slate-700 transition-colors"
                  >
                    Clear Summary
                  </button>{" "}
                </div>
                ...
                <button
                  ...
                  className="mt-3 p-2 bg-indigo-600 hover:bg-indigo-500 rounded-md text-xs flex items-center shadow-sm text-white"
                >
                  {" "}
                  <ClipboardIcon className="w-3.5 h-3.5 mr-1.5" /> Copy
                  Summary{" "}
                </button>
              </div>
            )}
                ...
                <div className="flex-grow flex flex-col overflow-hidden mt-4">
                  <div className="flex justify-end mb-2 gap-2">
                    {parsedChannelAnalysis &&
                      ...
                      !isSummarizingChannelAnalysis && (
                        <button
                          ...
                          className="p-2 bg-indigo-600 hover:bg-indigo-500 rounded-md text-xs flex items-center shadow-sm hover:shadow-lg transition-all text-white"
                    ...
                    <button
                      ...
                      className="p-2 bg-sky-600 hover:bg-sky-500 rounded-md text-xs flex items-center shadow-sm hover:shadow-lg transition-all text-white"
                    >
                      {" "}
                      <ClipboardIcon className="w-3.5 h-3.5 mr-1.5" />{" "}
                      {copied ? "Copied Analysis!" : "Copy Full Analysis"}{" "}
                    </button>
                  </div>
                  <div
                    className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto flex-grow pr-1 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-800/50"
                    style={{ maxHeight: "calc(100vh - 400px)" }}
                  >
                    {parsedChannelAnalysis.map((section, index) => (
                      <div
                        key={index}
                        className="bg-slate-800 p-4 rounded-lg shadow-lg hover:shadow-sky-500/30 transition-shadow duration-300 flex flex-col justify-between"
                      >
                        <div>
                          {" "}
                          ...
                          <p className="text-sm text-slate-300 mb-3">
                            {truncateText(section.content, 120)}
                          </p>{" "}
                        </div>
                        <button
                          onClick={() => setDetailedAnalysisSection(section)}
                          className="mt-auto self-start px-4 py-2 bg-sky-600 text-white text-sm rounded-md hover:bg-sky-500 transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50"
                        >
                          {" "}
                          View Details{" "}
                        </button>
          ...
          <aside className="md:w-2/5 lg:w-1/3 bg-slate-800/70 backdrop-blur-sm p-5 rounded-xl shadow-xl flex flex-col space-y-4 max-h-[calc(100vh-120px)]">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-semibold text-sky-400 flex items-center">
                <ListChecksIcon className="h-6 w-6 mr-2" />
                History
              </h3>
              {history.length > 0 && (
                <button
                  onClick={handleClearAppHistory}
                  className="px-2.5 py-1 bg-red-700 hover:bg-red-600 text-white text-xxs font-medium rounded-md flex items-center transition-colors shadow"
                >
            ...
            <div className="flex-grow overflow-y-auto space-y-3 pr-1 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700/50">
              {history.length === 0 && (
                <p className="text-slate-400 text-sm">
                  No history yet. Generated content will appear here.
                </p>
              )}
              {history.map((item) => (
                <div
                  key={item.id}
                  className={`p-3 rounded-lg border transition-all ${viewingHistoryItemId === item.id ? "bg-slate-600 border-sky-500 shadow-md" : "bg-slate-700/60 border-slate-600 hover:border-slate-500"}`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <button
                        onClick={() => handleViewHistoryItem(item)}
                        className="text-sm font-medium text-sky-400 hover:text-sky-300 mb-1 text-left focus:outline-none"
                      >
                        {CONTENT_TYPES.find(
                          (ct) => ct.value === item.contentType,
                      ...
                      >
                        Input: {truncateText(item.userInput, 40)}
                      </p>
                    ...
                    <div className="flex space-x-1.5 shrink-0">
                      <button
                        ...
                        className={`p-1 rounded ${item.isFavorite ? "text-yellow-400 hover:text-yellow-300" : "text-slate-500 hover:text-yellow-400"}`}
        ...
        {activeTab === "strategy" && (
          <div className="flex-grow bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-6">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white font-semibold rounded-lg shadow-md disabled:opacity-60 flex items-center justify-center space-x-2 self-start"
            >
            ...
            {generatedStrategyPlan && (
              <div className="mt-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600/50 max-h-[calc(100vh-500px)] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700/80">
                ...
                <button
                  ...
                  className="mt-4 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs rounded-md flex items-center space-x-1.5"
          ...
          <div className="flex-grow bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-4">
            <div className="flex justify-between items-center">
              ...
              <h2 className="text-2xl font-semibold text-sky-400 flex items-center">
                <CalendarDaysIcon className="w-7 h-7 mr-3 text-sky-400" />
                Content Calendar
              </h2>{" "}
              <div className="flex items-center space-x-2">
                ...
                <span className="text-lg font-medium text-slate-200 w-36 text-center">
            ...
            <div className="grid grid-cols-7 gap-px bg-slate-600 border border-slate-600 rounded-md overflow-hidden">
              {" "}
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <div
                  key={day}
                  className="p-2 text-center font-semibold text-xs text-sky-300 bg-slate-700/50"
                >
                  {day}
                </div>
              ))}{" "}
            ...
            <p className="text-xs text-slate-400">
              Tip: Content Strategy Plan items are automatically added here.
              Click a day to add custom events.
            </p>
          </div>
        )}
        {activeTab === "trends" && (
          <div className="flex-grow bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-6">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white font-semibold rounded-lg shadow-md disabled:opacity-60 flex items-center justify-center space-x-2 self-start"
              >
              ...
              {history.map((item) => (
                ...
                >
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-slate-300 max-h-24 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-500 scrollbar-track-slate-600/50 pr-1">
                      {" "}
          ...
          <div className="flex-grow bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <select
                ...
                className="sm:w-1/3 p-3 bg-slate-700 border border-slate-600 rounded-lg focus:ring-sky-500 focus:border-sky-500 text-slate-100"
              >
              ...
              <button
                ...
                className="px-5 py-3 bg-sky-600 hover:bg-sky-500 text-white font-semibold rounded-lg shadow-md disabled:opacity-60 flex items-center justify-center space-x-2"
              >
                <SearchIcon className="h-5 w-5" />{" "}
            ...
            {searchResults.length > 0 && (
              <div className="space-y-3 max-h-[calc(100vh-300px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700/50">
                {" "}
                {searchResults.map((result, index) => (
                  <div
                    key={index}
                    className="p-3 bg-slate-700/60 border border-slate-600 rounded-lg"
                  >
                    {" "}
                    <a
                      href={result.uri}
                ...
                {canLoadMoreSearchResults && !isSearching && (
                  <button
                    ...
                    className="mt-3 px-4 py-2 bg-slate-600 hover:bg-slate-500 text-slate-200 text-sm rounded-md"
                  >
                    Load More Results
                  </button>
                )}{" "}
        ...
        {activeTab === "youtubeStats" && (
          <div className="flex-grow md:w-full bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl shadow-2xl flex flex-col space-y-6">
            <h2 className="text-2xl font-semibold text-sky-400 mb-1 flex items-center">
              <PlayCircleIcon className="w-7 h-7 mr-3 text-sky-400" />
              YouTube Channel Stats
            </h2>
            <p className="text-sm text-slate-300">
              Enter YouTube channel names or video URLs (comma-separated) to get
            ...
            <div className="space-y-3">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-sky-500 hover:from-blue-600 hover:to-sky-600 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out disabled:opacity-60 flex items-center space-x-2"
                >
              ...
              <div className="mt-6 p-5 bg-slate-900/60 rounded-xl shadow-inner border border-slate-700/50">
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    className="w-full sm:w-auto p-2 bg-slate-700 border border-slate-600 rounded-lg focus:ring-sky-500 focus:border-sky-500 text-slate-100"
                  >
                  ...
                  <table className="w-full text-left table-auto border-collapse">
                    <thead>
                      <tr className="bg-slate-700">
                        <th className="p-3 border-b border-slate-600 text-slate-300">
                          Channel Name
                        </th>
                        <th className="p-3 border-b border-slate-600 text-slate-300">
                          Subscribers
              ...
              <div className="flex-grow bg-slate-900/60 p-5 rounded-xl shadow-inner overflow-y-auto min-h-[200px] border border-slate-700/50 relative scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-800/50">
                ...
                <div className="space-y-4">
                  {youtubeStatsData.map((entry) => (
                    <div
                      key={entry.id}
                      className="p-4 bg-slate-800 rounded-lg shadow-md border border-slate-700"
                    >
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="font-semibold text-sky-400">
                          Input: {truncateText(entry.userInput, 80)}
                        </h4>
                        <div className="flex space-x-2">
                          <button
                            onClick={() => handlePinYoutubeStatsToCanvas(entry)}
                            className="p-1.5 bg-slate-600/50 text-slate-400 hover:text-teal-400 rounded-md"
                            title="Add to Canvas"
                          >
                            <PlusCircleIcon className="h-4 w-4" />
                      ...
                      <div className="styled-text-output space-y-2">
                        {parseAndStyleText(entry.content)}
                      </div>
                      ...
                      <button
                        ...
                        className="mt-4 px-3 py-1.5 bg-teal-600 hover:bg-teal-500 text-white text-xs rounded-md flex items-center space-x-1.5"
                        title="Copy output text"
                      >
                        <ClipboardIcon className="h-4 w-4" />
      ...
      {/* Sign Out Button - Bottom Left Corner */}
      <button
        onClick={async () => {
        ...
        className="fixed bottom-4 left-4 bg-slate-800/90 hover:bg-slate-700 border border-slate-600 text-slate-300 hover:text-white p-3 rounded-lg shadow-xl z-50 transition-all duration-200 backdrop-blur-sm group"
        title="Sign Out"
      >
        <div className="flex items-center space-x-2">
          <UserCircleIcon className="h-5 w-5" />
